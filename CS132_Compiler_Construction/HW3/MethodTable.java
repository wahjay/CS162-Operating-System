...
class MethodTable {
  ...
  public int getFPosition(String cn, String fn) {
    ...
    ////////////////////////////////////////
    if(pos == -1) {
      for(String entry : fields_) {
        if(entry.substring(entry.indexOf(".")+1).equals(fn)) {
          field = entry;
          break;
        }
      }

      //retrieve super class name and method name
      //and then find its pos in the super class field
      pos = field.indexOf(".");
      String class_name = field.substring(0, pos);
      String method_name = field.substring(pos+1);
      fields_ = fields.get(class_name);
      pos = fields_.indexOf(class_name + "." + method_name);
    }
    ////////////////////////////////////////
  ...
  public int getMPosition(String cn, String mn) {
    ...
    if(pos == -1) {
      for(String entry : methods) {
        if(entry.substring(entry.indexOf(".")+1).equals(mn)) {
          method = entry;
          break;
        }
      }
      ...
      methods = methodTable.get(class_name);
      pos = methods.indexOf(class_name + "." + method_name);
    }
    ////////////////////////////////////////
